...
*Estrutura definitiva (2026-07-28). Desde 2026-08-04, cada missão é um caso de investigação independente (contexto + narrativa), substituindo o conceito anterior de "uma missão por grupo zoológico". A espécie-alvo deixou de ser conteúdo fixo da missão nesse mesmo dia — ver a nota de 2026-08-04 (b), abaixo.*
...
| `sempreDisponivel` | boolean | Não (padrão `false`) | Quando `true`, a missão nunca fica bloqueada nem mostra "concluída" (o que a tornaria não clicável), independente de progresso ou de `preRequisito`. Nenhuma missão usa isso hoje. |
...
| `visivelNoMapaDeMissoes` | boolean | Não (padrão `true`) | Quando `false`, a missão não aparece na listagem do Mapa de Missões (`listarMissoes()`), mas continua acessível por id via `obterMissao`. Nenhuma missão usa isso hoje. |
...
| `especieRespostaCorreta` | string ou null (FK → Espécie) | Não | **Reservado, sempre `null` no conteúdo atual** (ver nota 2026-08-04 (b) abaixo) — chegou a ser usado brevemente para fixar uma espécie por missão; hoje a espécie-alvo é escolhida pelo jogador em tempo de jogo (tela de Seleção de Espécime), não faz parte do conteúdo da missão. |
| `imagemPrincipalIndice` | number ou null | Não | **Reservado, sempre `null` no conteúdo atual**, pelo mesmo motivo acima. |
...
**Nota (2026-08-04 (a)):** o conceito de "uma missão por grupo zoológico" foi substituído por um conjunto de 12 casos de investigação independentes (`caso-*`), cada um representando só um contexto/cenário (residência, escola, trilha...) — várias missões compartilham `grupoId`/`perguntaInicialId` (ex.: 4 casos de aranhas, todos partindo de `"aranhas-p1"`).

**Nota (2026-09-14):** os 12 casos deixaram de formar uma única campanha linear atravessando os 3 grupos (a versão original desta nota descrevia essa cadeia única). O jogador agora escolhe o grupo antes do Mapa de Missões, numa tela de Seleção de Grupo (`js/telas/selecaoGrupo.js`) — para que os 3 grupos fiquem sempre escolhíveis, o `preRequisito` da primeira missão de Escorpiões e de Serpentes deixou de apontar para a última missão do grupo anterior (`caso-jardim-aranha` e `caso-urbano-escorpiao`, respectivamente) e virou `null`. Dentro de cada grupo a progressão continua igual: 4 casos encadeados por `preRequisito` na ordem de `ordemProgressao`. `js/telas/mapaMissoes.js` passou a receber `dados.grupoId` (escolhido na Seleção de Grupo) e a filtrar as missões por ele (`listarMissoesPorGrupo`, em `nucleo/missoes.js`/`database.js`) — nenhuma lógica de status foi duplicada, o filtro só reduz de quais missões ela é calculada.

**Nota (2026-08-05):** a Missão de Treinamento (`missao-0`, um tutorial jogável que reaproveitava a árvore de perguntas de Escorpiões) foi removida do conteúdo. Em seu lugar, o botão "Como Jogar" da Tela Inicial abre `js/telas/comoJogar.js` — um tutorial estático de 4 páginas (uma imagem anotada por página, sem árvore de perguntas nem espécie envolvida). Não é mais uma Missão: não tem entrada em `missoes.json`, não passa por `preRequisito`/`status`, e sua conclusão não grava em `progressoMissoes` (ver seção 9, "Tutorial visto").

**Nota (2026-08-04 (b), mesmo dia, revisão posterior):** a primeira versão dessa mudança fixava uma `especieRespostaCorreta` por missão (com sua `imagemPrincipalIndice`). Isso foi revertido no mesmo dia: a espécie-alvo agora é escolhida pelo próprio jogador, numa tela de Seleção de Espécime exibida entre a Introdução da Missão e a Investigação (listando as espécies do `grupoId` da missão, sem revelar o nome — só "Espécime 1", "Espécime 2"...). A escolha nunca é gravada em `missoes.json`: vive só como estado de sessão (`dados.especieEscolhidaId`, passado de tela em tela pelo motor de navegação), a mesma técnica já usada para `perguntaInicialId`. Ao final da investigação, `js/telas/investigacao.js` compara o resultado real do Motor (`especieId`) com `especieEscolhidaId`: só quando coincidem a identificação é tratada como correta, e só nesse caso o progresso é registrado (ver seção 9). Uma identificação incorreta oferece "Tentar novamente" (reinicia a árvore, mesma espécie escolhida) ou "Visualizar a resposta correta" (revela a espécie que deveria ter sido identificada, com a Ficha Científica e as características decisivas) — nenhuma das duas grava progresso. `especieRespostaCorreta`/`imagemPrincipalIndice` voltaram a ficar reservados para uma eventual necessidade futura de fixar um caso específico.

**Relacionamentos:** uma Missão referencia um Grupo e uma Pergunta (entrada na chave). Não referencia mais uma Espécie diretamente — a associação passou a ser decidida em tempo de jogo, não no conteúdo.
...
| `criterio.tipo` | enum: `"identificar_especie"` \| `"completar_grupo"` \| `"completar_missoes_fase"` \| `"catalogar_grupo"` \| `"catalogar_risco"` \| `"concluir_tutorial"` \| `"sequencia_acertos"` | Sim | Tipo de regra que desbloqueia a conquista. |
...
**Nota de implementação (2026-07-30, revisada em 2026-08-04 e 2026-08-05):** o conteúdo atual (13 conquistas, revisado para cobrir categorias distintas de progresso — catalogação, especialização por grupo, especialização por risco médico, missões e exploração geral) usa seis dos sete tipos de critério: `"identificar_especie"` (quantidade de espécies catalogadas, de qualquer grupo), `"completar_grupo"` (missão de grupo concluída), `"completar_missoes_fase"` (concluiu ao menos uma missão de grupo), `"catalogar_grupo"` (**todas** as espécies de um grupo catalogadas — mais forte que `"completar_grupo"`, que só olha a missão), `"catalogar_risco"` (todas as espécies de um grau de importância médica catalogadas) e `"concluir_tutorial"` (concluiu o tutorial "Como Jogar" — não depende de nenhuma missão real ter sido jogada; ver seção 6, nota 2026-08-05). Nenhuma conquista usa `"sequencia_acertos"` — exige um conceito de acerto/erro que a chave dicotômica determinística não tem. Avaliação em `js/nucleo/progressoCientifico.js` (`avaliarConquista`), a partir de `especiesDescobertas` (seção 9) e da flag "Tutorial visto" (seção 9), cruzados com o conteúdo de Grupo/Espécie/Missão.
...
**Tutorial visto (implementado em 2026-08-05):** flag booleana única, guardada na store `configuracoes` sob a chave `tutorialVisto` (mesmo padrão de `hashesConteudo` abaixo — bookkeeping, não conteúdo, não documentado como campo de `Configuração` na seção 8). Gravada por `salvarTutorialVisto()` (`database/scripts/indexeddb.js`) quando o jogador chega à última página do tutorial "Como Jogar" e clica em "Começar investigação". Lida em dois pontos: por `js/telas/telaInicial.js`, para decidir se o botão "Missões" abre o tutorial primeiro (primeiro acesso) ou vai direto ao Mapa de Missões; e por `obterProgressoCientifico()`, para avaliar a conquista "Primeiros Passos" (`criterio.tipo: "concluir_tutorial"`, seção 7). O botão "Como Jogar" da Tela Inicial nunca consulta essa flag — é sempre o replay manual do tutorial, disponível a qualquer momento.
